...
### Q8. VelocityBiasOverride semantics: is it strictly additive (int to add) or can it be multiplicative? Which interpretation should we implement for Story D2?
**Answer:** Implement **both**, deterministically, as a two-part bias:
- `Multiplier` (double, default **1.0**)
- `Additive` (int, default **0**)

If your current `GroovePolicyDecision` only has a single `VelocityBiasOverride` field, treat it as **Additive** for now (until you extend the model).

---

### Q9. Override application order: should the policy override apply before or after AccentBias is added?
**Answer:** Apply the override **after** the role+strength lookup has been resolved into a base value.

Deterministic order:
1. `base = Typical + AccentBias` (or ghost base; see Q10)
2. `biased = round(base * Multiplier) + Additive`
3. `clamp` (see Q13 + Q17)

This makes policy bias act as a true “final shaping layer” without distorting the accent model.

---

### Q10. Ghost handling precedence: if RoleGhostVelocity[role] exists AND RoleStrengthVelocity[role] contains an entry for OnsetStrength.Ghost, which should be used?
**Answer:** `RoleGhostVelocity[role]` **wins**.

Precedence for `OnsetStrength.Ghost`:
1. If `RoleGhostVelocity[role]` exists → use it as the **Typical** for ghost, with `AccentBias = 0`
2. Else if `RoleStrengthVelocity[role][Ghost]` exists → use that rule
3. Else → fall back (Ghost maps to Offbeat fallback logic; see Q11)

---

### Q11. Fallback defaults: when lookup fails (role missing or strength missing), what exact default should be used?
**Answer:** Never throw; use deterministic layered fallback.

**Fallback resolution order**
- If the role exists but the strength is missing:
  1. Try `OnsetStrength.Offbeat` for that role (neutral proxy)
  2. If Offbeat missing, pick the **first available** strength entry for that role using fixed priority:
     `Downbeat > Backbeat > Strong > Pickup > Offbeat` (first that exists)
- If the role is missing entirely:
  - Use global default `VelocityRule`:
    - `Typical = 80`, `AccentBias = 0`, `Min = 1`, `Max = 127`
- For Ghost specifically:
  - If no ghost rule exists, treat Ghost as Offbeat under the same fallback rules above.

---

### Q12. Data types & rounding: are bias and AccentBias integers only? If fractional/multiplicative bias is supported later, how should rounding be handled?
**Answer:** For Story D2:
- Stored rule values are **ints**: `Typical`, `AccentBias`, `Min`, `Max`
- Policy override multiplier is **double** (future-proof)

Deterministic rounding rule:
- After multiplication, round to nearest int with midpoint **AwayFromZero**
- Then apply additive

Formula:
- `biasedInt = (int)Math.Round(base * multiplier, MidpointRounding.AwayFromZero) + additive;`

---

### Q13. Min/Max enforcement scope: clamp to rule Min/Max OR also global MIDI 1..127?
**Answer:** **Both.** Deterministic clamp order:
1. Normalize rule bounds into legal MIDI domain (see Q17):
   - `ruleMin = Clamp(ruleMin, 1, 127)`
   - `ruleMax = Clamp(ruleMax, 1, 127)`
2. Clamp velocity to `[ruleMin..ruleMax]`
3. Clamp again to `[1..127]` (defensive; usually redundant after step 1)

---

### Q14. Diagnostics: what minimal diagnostic fields are required per-onset for velocity shaping (rule used, pre-clamp value, override applied)?
**Answer:** Minimal per-onset fields worth capturing (and safe to lock tests against):

- `Role`
- `Strength` (computed or overridden strength)
- `RuleSource` (enum/string): `RoleStrength | RoleGhost | FallbackRoleOffbeat | FallbackRoleFirst | FallbackGlobal`
- `Typical`
- `AccentBias`
...
That explains every output number without bloating the trace.

---

### Q15. Interaction with existing onset velocity: if an onset already has a velocity set (from anchors), should velocity shaping override it always, or only for variation-added onsets?
**Answer:** Do **not** override existing velocities.
- If `GrooveOnset.Velocity` is already set → **leave unchanged**
- If null/unspecified → apply velocity shaping

If you later want “global reshaping,” make it an explicit opt-in policy flag (not part of Story D2).

---

### Q16. Concurrency / mutation: is it acceptable to update the Velocity in place, or should a new immutable output record be produced?
**Answer:** Produce a **new immutable output record** (copy-with). No in-place mutation.

Example approach:
- `FinalOnsets = oldOnsets.Select(o => o with { Velocity = computed }).ToList();`

This is cleaner for determinism, avoids shared-list mutation bugs, and is thread-safe by default.

---

### Q17. If Min or Max are out of MIDI range, should they be normalized to [1..127] or treated as configuration error?
**Answer:** Normalize to `[1..127]` at runtime and surface a diagnostic/validation issue (non-throwing).

Deterministic normalization rules:
- `ruleMin = Clamp(ruleMin, 1, 127)`
- `ruleMax = Clamp(ruleMax, 1, 127)`
- If after clamp `ruleMin > ruleMax`, **swap them** (simplest deterministic behavior; least surprising).
